...
Never put `@test` inside a `for` loop (or any per-item loop) — it silently inflates the reported test count without adding meaningful coverage, and makes failures harder to localize. Instead, collect each iteration's boolean result into a `Vector{Bool}` (or similar) inside the loop, then assert once after the loop with `@test all(...)` (or `@test any(...)` when only one match is required). The goal is maximum meaningful coverage per assertion, not a high test count.
...
0. **`BitCodes`** (`src/bit_codes.jl`) — the shared bit-packing vocabulary every other module encodes against. Holds the canonical 2-bit strand codes (`STRAND_FWD` = 0, `STRAND_REV` = 1, `STRAND_BOTH` = 2, `STRAND_NA` = 3), conversions to/from `GenomicFeatures.Strand` and strand characters (`strand_code`/`decode_strand`/`strand_char`/`get_strand`), and the generic field helpers (`get_field`/`set_field`/`field_mask`/`clamp_field`) used to read and write packed codes. Two bits is the narrowest strand field any layout has (the methylation payload), so every layout uses that encoding; wider slots just leave their upper bits zero. **Note `0x00` means forward, not unknown** — a zeroed code decodes to `+`, so always write a strand explicitly (`STRAND_NA` when there isn't one). Also holds the pack/parse helpers for `Reference`'s 64-bit feature metadata code — `pack_metadata`/`parse_index`/`parse_strand`/`parse_so_term` — so all bit-layout logic lives in one place; `Reference` re-exports them as aliases (same pattern as `get_strand`) so existing call sites are unaffected. Add new packed fields here rather than hand-rolling shifts at the call site.
...
2. **`Reference`** (`src/reference.jl`) — the core in-memory genome representation. Key types: `Species` → `Genome` → `Scaffold` → interval tree of features. Each feature interval carries a packed 64-bit `code` (see the bit-layout diagram in the `Genome` docstring, packed/parsed via `BitCodes`) encoding an SO term + strand + an index into a separate string-interning metadata store (`vocab`/`meta_offsets`/`meta_blob`). This packed-code + interned-string design is deliberate for memory efficiency when loading whole-genome GFF3 annotations — don't casually replace it with a `Dict`-of-structs without understanding why. `add_features!` parses a GFF3 file (optionally gzipped) using a producer/consumer pattern: the main task parses records into batches on a `Channel`, a spawned task commits them into the `Genome`. `Genome`/`Scaffold` support ID-based lookup via `Base.getindex` (single ID, a `Vector` of IDs for bulk lookup, or a metadata index), which is an O(features) linear scan by design (no ID index is maintained) — see the docstrings for the rationale. A looked-up feature comes back as a `FeatureRecord`, which (besides ID/type/coordinates/metadata) also carries the raw 64-bit `code`, so callers needing the packed strand/SO-term/metadata-index don't have to re-resolve it. `IntervalSimple` is a minimal `(start_pos, end_pos, code)` struct — no scaffold/vocab attached — used where only the bare interval + code are needed (e.g. `Paralogs.GeneFamily`'s per-scaffold `StructArray`s).
...
4. **`Paralogs`** (`src/paralogs.jl`) — depends on `Reference` (for `Genome`/`IntervalSimple`), plus `Graphs`, `SimpleWeightedGraphs`, `SparseArrays` and `StructArrays`. Centres on `GeneFamily`: built from a `Reference.Genome` and a paralog-pair `DataFrame` (2–6 columns — mandatory query/subject ID, then optional `dN`/`dS`/`id_subject_query`/`id_query_subject` matched *by column name*, case-insensitively). Genes referenced by the pair table are resolved against the genome, grouped by scaffold, and given a contiguous linear index (`scaffold_ranges`/`intervals`/`id_to_index`); every relation is a `SparseMatrixCSC` over that index, each oriented so the axis you look up by is the fast (column-major) one — `topology` is a symmetric `Bool` adjacency, `dN`/`dS` are symmetric so stored **once** in the upper triangle (`[min(a,b), max(a,b)]`), and `id_subject_query`/`id_query_subject` are directed (subjects vs. queries on the columns, respectively). Rows with a `NaN` in any matched value column are dropped up front with a warning — otherwise `NaN != NaN` would break `dN`/`dS`'s `*_graph` symmetry checks. `GeneFamily` has exactly one *public* constructor (an unexported `_RawFields` marker gates the raw-fields one used internally by sub-family `getindex`), plus `getindex` (`gf[i]`/`gf["id"]` → a column dict; `gf[indices]`/`gf[ids]` → a sub-family `GeneFamily`) and `Base.show`. `topology_graph`/`dN_graph`/`dS_graph`/`id_subject_query_graph`/`id_query_subject_graph` convert the relation matrices to `Graphs`/`SimpleWeightedGraphs` views (dN/dS are symmetrised — `m + permutedims(m)` — since `SimpleWeightedGraph` requires a fully symmetric matrix; the two directed relations pass straight to `SimpleWeightedDiGraph`). `rbh` has two methods: the original `rbh(df::DataFrame; scoring)` (percent-identity scoring: max/mean/double_max, plus `rbh_ds` for dS-based scoring) works on a flat pair table with no genome context; `rbh(gf::GeneFamily; scoring)` instead finds `gf.topology`'s connected components and keeps each component's single lowest-distance edge (ranked ascending by `dS`, then `dN`, then `%ID`, skipping whichever levels `gf` lacks), warning about and deterministically resolving ties.

5. **`Plotting`** (`src/plotting.jl`) — currently a near-empty stub (`using CairoMakie` only, no functions yet).

6. **`Exploration`** (`src/exploration.jl`) — statistics built on `Reference`/`Data`: per-feature `coverage` fractions from `BedData`, `kde` density estimates (on coverage fractions or on `TabularData`), three `quantiles` overloads — one value-based on `TabularData` (collapses each row via `merge`, default `mean`, then bins by the *value* quantiles of those merged scalars), one rank-based on `TabularData` (`quantiles(data, ranking::Vector{String})`, cascading tie-breaks over named variables then original sample order), and one rank-based on a plain `DataFrame` (`quantiles(pairs::DataFrame, ranking::Vector{String})`, the same ranking scheme applied to a paralog-pair table shaped like `Paralogs.GeneFamily`'s constructor input or `Paralogs.rbh`'s output, appending a `"quantile"` bin column) — and a genome-wide-coverage pipeline: `calculate_frequency` (per-base overlap counts across many `BedData` measurements, parallelized over scaffolds with `Threads.@threads` using a difference-array sweep) → `feature_frequency` (projects that per-base frequency onto each feature ± a flank, strand-oriented so index 1 is the 5' end) → `gene_profile`/`mean_gene_profile` (interpolates each feature's body to a fixed bin count and averages into a metagene profile).
...
- The 64-bit `value` on an interval is always a packed metadata code; use `BitCodes.parse_index`/`parse_strand`/`parse_so_term` (re-exported by `Reference`; gene features) or `Data.parse_bed_strand` (BED features) to unpack it rather than bit-twiddling inline. Methylation payloads are unpacked the same way, via `Methylation.get_depth`/`meth_percent`/`get_meth`/`get_unmeth`/`get_context`/`is_forward` — prefer `get_depth`/`meth_percent` there, as those are the stored fields and the counts are reconstructions.
...
- `quantiles` lives entirely in `Exploration` (all three overloads, including the `DataFrame` one useful for paralog-pair tables) rather than being split across `Exploration`/`Paralogs`. Two sibling modules each independently declaring `function quantiles(...)` and exporting it produces an ambiguous binding the moment both are loaded in the same session (as every `TESTS_*.jl` file is, via `runtests.jl`) — it breaks at call time, not compile time, so don't reintroduce a same-named function in another module without either importing and extending the existing generic or picking a different name.
